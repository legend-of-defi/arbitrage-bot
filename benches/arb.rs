...
fn generate_benchmark_pools(pool_count: usize, token_count: usize) -> Vec<Pool> {
    ...
    for i in 0..pool_count {
        ...
        let idx1 = rng.gen_range(0..token_count);
        let mut idx2 = rng.gen_range(0..token_count);

        // Ensure tokens are different
        while idx1 == idx2 {
            idx2 = rng.gen_range(0..token_count);
        }

        // Create pool with random reserves
        let reserve0 = U256::from(rng.gen_range(1000..1_000_000));
        let reserve1 = U256::from(rng.gen_range(1000..1_000_000));
        ...
        if i < 3 || i == pool_count - 1 {
            println!("  Pool {}/{}: ID={}, token0={}, token1={}, reserve0={}, reserve1={}",
                     i+1, pool_count, pool.id, pool.token0, pool.token1, reserve0, reserve1);
        } else if i == 3 {
...
pub fn find_affected_cycles(pools: &Vec<Pool>, updated_pool: Pool) -> Vec<Cycle> {
    ...
    for pool in pools.iter().chain(std::iter::once(&updated_pool)) {
        ...
        token_graph.entry(token0.clone())
            .or_insert_with(Vec::new)
            .push((token1.clone(), pool.id.clone(), true)); // token0 -> token1

        token_graph.entry(token1)
            .or_insert_with(Vec::new)
    ...
    let profitable_cycles = all_cycles.clone().into_iter()
        .filter(|cycle| {
    ...
    if !profitable_cycles.is_empty() {
        ...
        for (i, swap) in cycle.swaps.iter().enumerate() {
            println!("Swap {}: {} -> {}",
                     i+1, swap.token_in(), swap.token_out());
        }
    }
...
fn dfs_find_cycles(
    ...
    if depth > 0 && current_token == start_token {
        ...
        for i in 0..path.len() {
            ...
            // Create swap ID
            let direction = if *is_forward { Direction::ZeroForOne } else { Direction::OneForZero };
            let swap_id = SwapId {
        ...
        if cycle_contains_updated_pool && !swaps.is_empty() {
            ...
            if let Some(min_pos) = swap_ids.iter()
                .enumerate()
...
fn bench_find_cycles(c: &mut Criterion) {
    ...
    for pool_count in [100, 500, 1000].iter() {
        ...
        println!("Selected updated pool: ID={}, token0={}, token1={}",
            ...
            |b, _| {
                ...
                b.iter_batched(
                    ...
                    |(p, up)| {
                        ...
                        metrics.max_cycle_length = cycles.iter()
                            .map(|c| c.swaps.len())
                            .max()
                            .unwrap_or(0)
                            .max(metrics.max_cycle_length);
                        metrics.avg_execution_time_ms =
                            (metrics.avg_execution_time_ms * metrics.samples as f64 +
                                duration.as_millis() as f64) / (metrics.samples + 1) as f64;
                        metrics.samples += 1;
        ...
        println!("Total profitable cycles found: {}", metrics.profitable_cycles);
        println!("Avg cycles per sample: {:.2}",
...
fn bench_production_data(c: &mut Criterion) {
    ...
    for (name, pool_count, token_count, density) in test_configs {
        println!("\n========================================================");
        println!("PRODUCTION TEST: {}, {} pools, {} tokens, density {:.1}",
        ...
        println!("High connectivity pool: ID={}, token0={}, token1={}",
                 high_connectivity_pool.id, high_connectivity_pool.token0,
                 high_connectivity_pool.token1);

        // 2. Update a low-connectivity pool (few cycles affected)
        let low_connectivity_pool = pools[pools.len()-1].clone();
        println!("Low connectivity pool: ID={}, token0={}, token1={}",
                 low_connectivity_pool.id, low_connectivity_pool.token0,
                 low_connectivity_pool.token1);
            ...
            |b, _| {
                b.iter_batched(
                    ...
                    |(p, up)| {
                        ...
                        println!("HIGH CONNECTIVITY: Found {} profitable cycles in {:?}",
                                 cycles.len(), duration);
            ...
            |b, _| {
                b.iter_batched(
                    ...
                    |(p, up)| {
                        ...
                        println!("LOW CONNECTIVITY: Found {} profitable cycles in {:?}",
                                 cycles.len(), duration);
...
mod tests {
    ...
    fn create_test_pool(id: &str, token0: &str, token1: &str, reserve0: u64, reserve1: u64) -> Pool {
        Pool::new(
    ...
    fn test_example_graph() {
        // Create pools with rates that make (1,2,3) cycle profitable
        // but (1,2,5,4) cycle unprofitable

        // Note: We'll define rates so path 1->2->3->1 is profitable (positive log rate)
        // but path 1->2->5->4->1 is not profitable (negative log rate)
        let edge1 = generate_random_address().as_str();
        let edge2 = generate_random_address().as_str();
        let edge3 = generate_random_address().as_str();
        let edge4 = generate_random_address().as_str();
        let edge5 = generate_random_address().as_str();
        let token0 = generate_random_address().as_str();
        let token1 = generate_random_address().as_str();
        let token2 = generate_random_address().as_str();
        let token3 = generate_random_address().as_str();
        let pools = vec![
            create_test_pool(edge1, token0, token1, 1000, 1000),
            create_test_pool(edge2, token0, token2, 1000, 1000),
            create_test_pool(edge3, token1, token2, 1000, 1000),
            create_test_pool(edge4, token1, token3, 10000, 10000),
            create_test_pool(edge5, token2, token3, 30000, 300000),
        ];

        println!("pools: {:?}", pools);

        // Update Edge1
        let updated_pool = create_test_pool(edge1, token0, token1, 100, 210);

        // Find profitable cycles affected by the update
        let cycles = find_affected_cycles(&pools, updated_pool);

        assert!(!cycles.is_empty(), "Should find at least one profitable cycle");

        // We expect to find cycles that contain the updated pool and are profitable
        for cycle in &cycles {
            // Verify cycle contains the updated pool
            let cycle_contains_updated_pool = cycle.swaps.iter().any(|swap|
                swap.id().pool_id.to_string() == edge1
            );
            assert!(cycle_contains_updated_pool, "Cycle should contain the updated pool");

            // Verify cycle is profitable
            assert!(cycle.is_positive(), "Cycle should be profitable");

            // Print cycle for debugging
            println!("Found profitable cycle: {:?}", cycle);
        }

        // Make sure we don't have excessive cycles
        assert!(cycles.len() <= 4, "Should not find more than 4 cycles in this graph");
    }
